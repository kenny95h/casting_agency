...
* Base URL:
  ...
  * If running locally, the app is hosted at the default, `http://127.0.0.1:5000/`, and is a Flask-based app. To install the dependencies `cd` to the casting_agency folder, then `pip install -r requirements.txt`, and then start the Flask app by setting the env `export FLASK_APP=app`, then `export FLASK_ENV=development`, and run the app with `flask run`
